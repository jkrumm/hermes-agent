...
| `tools/checkpoint_manager.py` | `checkpoint-gitlink-recovery` | `_take()`'s `git add -A` no longer hard-fails forever on a stale gitlink: on an add failure naming a `.git`, drop index gitlinks whose nested repo no longer resolves (`update-index --force-remove`) and retry once. A nested repo recorded while healthy survives `_seed_project_index`'s read-tree every snapshot, so one `com.apple.tmp_cleaner` pass over its `.git` wedged every later checkpoint of that project |
