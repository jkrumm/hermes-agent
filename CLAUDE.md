...
- `~/.hermes/hermes-agent/agent/auxiliary_client.py` — in `_build_call_kwargs`, send `max_completion_tokens` for `gpt-5*`/`gpt-4o`/`o-series` models by NAME (not only when the host is `api.openai.com`). Source: `patches/auxiliary-client-gpt5-max-completion-tokens.patch`. Re-apply: `cd ~/.hermes/hermes-agent && git apply ~/SourceRoot/hermes-agent/patches/auxiliary-client-gpt5-max-completion-tokens.patch`. Without this, the auxiliary model `gpt-5-mini` (served on the IU unified endpoint, not `api.openai.com`) is sent `max_tokens` and the endpoint returns HTTP 503 — so every aux task (title/compression/approval/web-extract) fails. Mirrors the same patch in Feuer (applied at Docker build).

- **TTS/STT use the stock upstream tools** — no patch. `tools/tts_tool.py` (native `openai` provider) and `tools/transcription_tools.py` (native `openai` STT) are pointed at audio-proxy (`:7716`) purely via `config.yaml` (`tts.openai` / `stt.openai`). The old localai-helper client patches (`tts_tool.py`, `tts_fast_tool.py`) and the `toolsets-expose-text-to-speech-fast` patch were removed when Hermes moved to Gemini Charon via audio-proxy. **After `hermes update`, do nothing for audio** — the upstream files are correct as-is; just confirm `config.yaml` still points `tts.openai.base_url` / `stt.openai.base_url` at `http://127.0.0.1:7716/v1`.
