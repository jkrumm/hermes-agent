...
- `~/.hermes/hermes-agent/agent/auxiliary_client.py` — in `_build_call_kwargs`, send `max_completion_tokens` for `gpt-5*`/`gpt-4o`/`o-series` models by NAME (not only when the host is `api.openai.com`). Source: `patches/auxiliary-client-gpt5-max-completion-tokens.patch`. Re-apply: `cd ~/.hermes/hermes-agent && git apply ~/SourceRoot/hermes-agent/patches/auxiliary-client-gpt5-max-completion-tokens.patch`. **Currently dormant:** the auxiliaries (title/compression/approval/web-extract) now run `DeepSeek-V4-Flash` (IU unified endpoint, Azure Spain Central / EU-resident), which accepts plain `max_tokens` — so the name-gated patch no longer fires for any configured model. Kept applied defensively: without it, any future `gpt-5*`/`gpt-4o`/`o-series` aux on the IU endpoint (not `api.openai.com`) would be sent `max_tokens` and the endpoint returns HTTP 503. Mirrors the same patch in Feuer (applied at Docker build).
