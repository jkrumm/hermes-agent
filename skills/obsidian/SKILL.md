...
- **Curated human surface — `Projects/`, `Areas/`.** The pages Johannes reads and writes — Area/Project folder notes (`{name}.md`, Folder Notes plugin) as overviews, plus human pages. Any language, **light** discipline: no forced `type`/`description`, `status` is his free field, and they link *down* into `wiki/` for depth rather than duplicating it. Curated pages are rewritten, never appended — see Guardrails. A page may be distilled from `wiki/` via Claude Code's `/distill` skill; the voice pass and publish decision are always human, never automated. Any prose you write for either layer follows the vault's root `voice.md` (writing-voice guide — verdict first, terse, no AI filler).
...
- **Always name the vault in the git command — `git -C ~/SourceRoot/brain …`.** Your shell's working directory is not the vault, so a bare `git commit` lands in the wrong repo or none. Easiest is the helper:
  ```bash
