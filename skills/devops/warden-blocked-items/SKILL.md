...
- **An implement episode cannot base on another branch — the brief cannot fix that.**
  Its worktree is cut fresh from the repository's **default branch**, and the tier's
  own rules forbid remote operations and touching another branch, so
  "commit on `<sha>`" / "push to the PR's branch" is *structurally* impossible
  there: it answers `no_changes`, `nextAction: human`, tree clean
  (*"the implement tier cuts from master and forbids touching another branch or
  remote ops, so 'commit on e671bf1' is structurally impossible here"*). Naming the
  base commit and the checkout mechanism in the brief does **not** help, and
  neither does the verdict saying it — the constraint is the tier prompt, not the
  text. So a fix that belongs to an *unmerged* carrier has exactly two routes:
  the owner merges the carrier PR first and the follow-up is briefed from master,
  or the carrier is abandoned and the change is re-derived from master in one PR.
  Both are one episode; the merge route keeps the already-reviewed diff, the
  re-derivation gets step-7 validation of its own. Surface that pair as the single
  owner decision and discharge the `needs_human` bounce with
  `close <event-id> --why`.
