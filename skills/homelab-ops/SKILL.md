...
| `redeploy <host> <stack>` | The running stack is behind the deployed truth, or containers were lost/orphaned | `homelab`: the `homelab` stack. `vps`: the three root stacks (`networking`, `infra`, `monitoring`) **plus every app stack** — `argo`, `audio-gateway`, `basalt-ui-marketing`, `bun-email-api`, `fpp`, `image-gen-gateway`, `imgproxy`, `meteo`, `photo-gallery`, `research-gateway`. An app stack shells the repo's own `make <stack>-up`, so it inherits that target's image pinning rather than reimplementing it. `homelab-private` is deliberately absent (its deploy is a full VPN cycle) |
