...
> blocks a direct `claude`/`claude_iu`/`claude_bridge`/`ca`/`opencode`/`rd`/`agent-dispatch`/
> `remote-dev.sh` invocation — and `herdr agent …` or `herdr pane run … claude …`, the two herdr
> shapes that place one — and points at the dispatcher. It handles wrappers (`timeout`, `env`, `nohup`, `sudo`, `xargs`, `nice`),
