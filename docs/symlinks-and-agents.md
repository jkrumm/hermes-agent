...
| `config/` | `~/.hermes/config/` | symlink — tracked agent-facing config. Held `dispatch-repos.json`, the dispatch policy `warden` resolves a repo against (root, `deny` list, default tier and per-repo ceilings), until that file moved to `warden/config/dispatch-repos.json` with the CLI on 2026-09-10 — the policy decides what an unattended episode may do, so it stayed deliberately in git rather than becoming runtime state, just under the new repo. This directory is empty now; kept for whatever tracked agent-facing config lands here next. |
| `skills/{name}/` | `~/.hermes/skills/{name}/` | symlink per skill — actual dirs are `capture`, `argo-api`, `work`, `karakeep`, `obsidian`, `reading`, `wildrift`, `research-gateway`, `image-delivery`, `homelab-ops`, `homelab`, `hermes-gateway`, `briefing-tts`, `claude-dispatch`, `rollhook-deploys`, `hyperdx`, `podcast`, `agents`, `human-queue`, `warden` (the former infrastructure/schedule/slack/tasks/weather/garmin-health/strength skills were consolidated into `argo-api/references/*.md` — now incl. `walking-pad.md`; they are no longer separate dirs and were dropped from `HERMES_SKILLS`). **`HERMES_SKILLS` in the Makefile is the source of truth — this list must match it (20 entries today).** `homelab` is additionally a **category dir**: `skills/homelab/{tailscale-diagnostics,torrent-stack-diagnostics}/` load as their own skills (`hermes skills list` shows them under category `homelab`) and ride the parent symlink, so they need no `HERMES_SKILLS` entry. Hermes authored both itself, through the symlink, during ordinary foreground turns — it also patches `references/` files in place the same way. Such edits are safe from the background curator (they resolve under `skills.external_dirs`) but **invisible to warden's `watchdog-poll.py` `stray_skill` source**, which skips symlinked top-level dirs and excludes anything under `external_dirs` regardless. Git is the only thing that sees them, which is why `make status` asserts `skills/` is committed. **This repo is public** — anything Hermes writes here gets read for tailnet names, node IPs, workspace user ids and `homelab-private` internals before it is committed (all four were present on the first pass). |
...
The alert-triage act-loop (`triage.py`, `com.jkrumm.warden-loop`, every 10 min) used to be a
third host-level script here — it moved to `~/SourceRoot/warden` on 2026-09-09, deliberately
still a LaunchAgent and not a `hermes cron` job, for the same reason: see
`~/SourceRoot/warden/docs/triage.md` § *Why a LaunchAgent, not `hermes cron`*.
...
`hermes-ops.log` and `warden-cli.log` (`hermes-cc.log` is the retired predecessor
name, still rotated until it ages out).
